CREATE TYPE public.app_role AS ENUM ('founder','boss','admin','franchise','vendor','reseller','developer','finance','sales','marketing','support','employee','author','seo','influencer','affiliate','customer','marketplace-user');
...
USING (user_id = auth.uid() OR public.has_role(auth.uid(),'admin') OR public.has_role(auth.uid(),'boss'));
